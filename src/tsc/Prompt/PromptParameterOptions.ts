...
import { PromptParameter, PromptParameterOption } from "src/prompt/Prompt";
...
export class PromptParameterOptions extends PromptParameter {
  ...
  constructor(
    ...
    options.forEach(kv => {
      this.options.push(
        new PromptParameterOption(kv[0], kv[1], kv[1] == value)
      );
    });
  }
  ...
  protected getEditor() {
    ...
    return $dom.el(
      `<select type="text" id="${this.key}" class="brickyeditor-input" placeholder="${this.placeholder}">${options}</select>`
    );
  }
}
